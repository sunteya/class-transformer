...
export class TransformOperationExecutor {
    ...
    transform(source, value, targetType, arrayType, isMap, level = 0) {
        if (Array.isArray(value) || value instanceof Set) {
            ...
            for (const key of keys) {
                ...
                const valueKey = key;
                let newValueKey = this.options.propertyFormat ? this.options.propertyFormat(key) : key, propertyName = this.options.propertyFormat ? this.options.propertyFormat(key) : key;
                ...
                if (!this.options.enableCircularCheck || !this.isCircular(subValue)) {
                    ...
                    if (this.transformationType === TransformationType.CLASS_TO_PLAIN) {
                        ...
                        finalValue = this.applyCustomTransformations(finalValue, targetType, transformKey, value, valueKey, this.transformationType);
                    ...
                    finalValue = this.applyCustomTransformations(finalValue, targetType, key, value, valueKey, this.transformationType);
                    if (finalValue !== undefined || this.options.exposeUnsetFields) {
                        if (newValue instanceof Map) {
                            newValue.set(newValueKey, finalValue);
                        }
    ...
    applyCustomTransformations(value, target, key, obj, valueKey, transformationType) {
